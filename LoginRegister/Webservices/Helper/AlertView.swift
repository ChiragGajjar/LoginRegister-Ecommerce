...
class AlertView: NSObject {
    ...
    {
        AlertView.showAlert(strTitle, strMessage: "", button: NSMutableArray(object: Language.OK.Localized()),viewcontroller : viewcontroller, blockButtonClicked: nil)
    }
    
    class func showOKMessageAlert(_ strMessage : String, viewcontroller : UIViewController)
    {
        AlertView.showAlert("Help", strMessage: strMessage, button:  NSMutableArray(object: Language.OK.Localized()),viewcontroller : viewcontroller, blockButtonClicked: nil)
    }
    
    class func   showMessageAlert(_ strMessage : String, viewcontroller : UIViewController)
    {
        AlertView.showAlert("", strMessage: strMessage, button:  NSMutableArray(object: Language.OK.Localized()),viewcontroller : viewcontroller, blockButtonClicked: nil)
    }
    
    class func showOKTitleMessageAlert(_ strTitle : String,strMessage : String, viewcontroller : UIViewController)
    {
        AlertView.showAlert(strTitle, strMessage: strMessage, button:  NSMutableArray(object: Language.OK.Localized()),viewcontroller : viewcontroller, blockButtonClicked: nil)
    }
    ...
    class func showActionSheetWithCancelButton(_ isCancelButton : Bool, title : String?,buttons:[String],viewcontroller : UIViewController, blockButtonClicked : buttonClicked?)
    {
        ...
        if isCancelButton {
            ...
            let cancelAction = AlertAction(title: Language.Cancel.Localized(), style: UIAlertAction.Style.cancel, handler: { (action) in
                //blockButtonClicked?((action as! AlertAction).tag!)
            })
